...
func wireApp(confServer *conf.Server, kafka *conf.Kafka, elasticsearch *conf.Elasticsearch, confData *conf.Data, logger log.Logger) (*kratos.App, func(), error) {
	...
	reader := job.NewKafka(kafka)
	elasticSearch, err := job.NewElastic(elasticsearch)
	if err != nil {
		cleanup()
		return nil, nil, err
	}
	jobWoker := job.NewJobWorker(reader, elasticSearch, logger)
	app := newApp(logger, grpcServer, httpServer, jobWoker)
	return app, func() {
		cleanup()
	}, nil
}
